...
Figure \@ref(fig:sales-time-series-plot) represents the sales time series of zero-calorie and sugary beverages across different sites, colored by the Intervention. Both sugary drink and zero calorie drink sales trend is different across different interventions, indicating that different interventions does have different impact on the sales. The sales trend is different across sites, and the effect of intervention appears to be different across sites as well. 
...
```{r Boxplots, fig.cap="The boxplots display the sales distribution of Sugary and Zero-Calorie beverages across various interventions. Each intervention is colour-coded and shows the range of sales data with the central line representing the median sales. Note that the plot only illustrates weekday sales.", echo=FALSE, fig.height=5, fig.width=8, fig.pos="p", fig.label='fig:boxplot', echo=FALSE,  message=FALSE, warning=FALSE}
...
# Assuming your dataframe is named `beverage_sales`
# and it includes a time variable `time` representing the measurement occasions
library(nlme)
...
In conclusion, our analysis demonstrates the value of integrating ITSA with LMM for evaluating public health interventions in shifting consumer behaviour. While our findings highlight the potential of certain interventions to positively influence consumer behaviour, they also point to the importance of considering temporal patterns in the design and implementation of health promotion strategies. The refined analysis, which incorporated interactions between interventions and time, revealed the progressive influence of exercise-related messaging of the calorie content on shifting consumer preferences towards zero-calorie beverages. Furthermore, the discounting strategies show a significant impact after commencing the intervention but fail to sustain the impact. Variations in preference between weekdays and weekends further highlighted the importance of timing and pointing to different target populations. 
...
```{r correlation, echo=FALSE, fig.align='center', fig.width=3.5}
